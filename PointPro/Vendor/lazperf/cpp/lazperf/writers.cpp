...
void basic_file::Private::writeHeader()
{
    ...
    {
        head14.vlr_count++;
        head14.point_offset += (uint32_t)(wktVlr.size() + wktVlr.header().Size);
    }
    if (head14.ebCount())
    {
    ...
    if (head14.version.minor == 4)
    {
        ...
        // Set/clear WKT bit based on whether we emit WKT VLR.
        if (!wkt.empty())
...
named_file::config::config() : scale(1.0, 1.0, 1.0), offset(0.0, 0.0, 0.0),
    chunk_size(DefaultChunkSize), pdrf(0), minor_version(3), extra_bytes(0), wkt("")
{}

named_file::config::config(const vector3& s, const vector3& o, unsigned int cs) :
    scale(s), offset(o), chunk_size(cs), pdrf(0), minor_version(3), extra_bytes(0), wkt("")
{}
...
named_file::config::config(const header12& h) : scale(h.scale.x, h.scale.y, h.scale.z),
    ...
    pdrf(h.point_format_id), minor_version(h.version.minor), extra_bytes(h.ebCount()), wkt("")
{}
